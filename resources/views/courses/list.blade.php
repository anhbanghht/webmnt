        	...
        	<div class="col-lg-12">
    			<div class="card card-underline">
    			    <div class="card-head card-head-lg">
						<header>Quản lý thông tin khóa học</header>
						<div class="tools">
							<div class="btn-group">
    						...
    						<table class="table no-margin" id="datatable1">
    							...
    							<tbody>
    							    ...
    							    <tr>
    									<td>{{$key+1}}</td>
    									<td>{{$item->course}}</td>
    									<td>{{$item->course_full}}</td>
    									<td>{{$item->note}}</td>
    									<td>
    									    @if( \Auth::user()->has_permission( 'courses::view' ) )
    									    <a href="{{route('courses::view',['id'=>$item->id])}}" class="btn ink-reaction btn-primary" data-toggle="tooltip" data-placement="top" data-original-title="Thông tin khóa học"><span class="glyphicon glyphicon-eye-open"></span></a>
    									    @endif
    									    @if( \Auth::user()->has_permission( 'courses::edit' ) )
    									    <a href="{{route('courses::edit',['id'=>$item->id])}}" class="btn ink-reaction btn-primary"><span class="glyphicon glyphicon-pencil"></span></a>
    									    @endif
    									    @if( \Auth::user()->has_permission( 'courses::delete' ) )
    									    <a href="{{route('courses::delete',['id'=>$item->id])}}" class="btn ink-reaction btn-primary remove-btn"><span class="glyphicon glyphicon-remove"></span></a>
    									    @endif
    									</td>
    								</tr>
    							    @endforeach
