...
<li>
	<a href="{{ route('assign::listassign',$items->id)}}" style="color:blue;">Danh sách chi tiết nhiệm vụ</a>
</li>
@stop
@section('active_work')
class="active"
	...
	<div class="section-body">
		...
		<div class="row">
			<div class="col-md-8">
				...
				<button type="button" class="btn btn-flat btn-primary" onclick="DoanListView.assignToday(<?php echo $items->id;?>)">Hôm nay</button>
				<button type="button" class="btn btn-flat btn-primary" onclick="DoanListView.assignToweek(<?php echo $items->id;?>)">7 Ngày tiếp theo</button>
				<button type="button" class="btn btn-flat btn-primary" onclick="DoanListView.assignTomonth(<?php echo $items->id;?>)">Tháng hiện tại</button>
				<div class="form-group">
					<div class="input-daterange input-group" id="demo-date-range">
						...
						<button type="button" class="btn btn-flat btn-primary" onclick="DoanListView.filterdate(this,<?php echo $items->id;?>)">Lọc</button>
					</div>
				</div>
			</div><!--end .col -->
		</div><!--end .row -->
									...
									<tbody>
										...
										<?php 
											foreach($assignyes as $task):
												if($task->workid == $items->id): $count++; endif;
											endforeach;
										?>
										<?php $num=0; ?>
										<?php foreach($assignyes as $task): 
											if($task->workid == $items->id): $num++;
										?>	
													...
													<div class="checkbox checkbox-styled tile-text">
														<label>
															<?php if($task->alltime != 0 ): ?>
																<input type="checkbox" value="1" checked="checked" onclick="DoanListView.choosevalue(this,'alltime',<?php echo $task->id;?>)" <?php if($items->typeid == 1): echo "disabled"; endif;?>>
															<?php else: ?>
																<input type="checkbox" value="1" onclick="DoanListView.choosevalue(this,'alltime',<?php echo $task->id;?>)" <?php if($items->typeid == 1): echo "disabled"; endif;?>>
															<?php endif; ?>
														</label>
													</div>
												</td>
