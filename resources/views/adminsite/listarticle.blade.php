...
<li>
	<a href="{{ route('site::listarticle')}}" style="color:blue;">Danh sách bài viết</a>
</li>
@stop
@section('active_article_list')
class="active"
@stop
...
<section class="style-default-bright">
	...
	<div class="section-body">
			...
			<div class="col-lg-12">
				<div class="table-responsive">
					<form enctype="multipart/form-data" class="form form-validate floating-label adminform" accept-charset="UTF-8" action="javascript:void(0)" method="POST">
						<?php echo csrf_field(); ?>
						<table id="article-view" class="table table-striped table-hover">
							<thead>
								<tr>
									<th>{{ trans('languages.id') }}</th>
									<th>{{ trans('languages.articlename') }}</th>
									<th>{{ trans('languages.description') }}</th>
									<th>{{ trans('languages.articlecontent') }}</th>
									<th>{{ trans('languages.image') }}</th>
									<th>{{ trans('languages.attached') }}</th>
							...
							<tbody>
								...
								<tr>
									...
									<td><?php if($item->image!=''):?><img src="../uploads/images/<?php echo $item->image;?>" style="width:100px;"><?php endif;?></td>
									<td><?php if($item->attached!=''):?><a href="../uploads/attachs/<?php echo $item->attached;?>"><?php echo $item->attached;?></a><?php endif;?></td>
									<td><?php echo $item->created_at; ?></td>
									<td>
										<a href="{{route('site::geteditarticle',$item->id)}}" >
											<i class="fa fa-pencil"></i>
										</a>
									</td>
									<td>
										<a href="javascript:void(0)" onclick="DoanListView.DeleteItem('article',<?php echo $item->id;?>)">
		...
		<!-- END DATATABLE 1 -->
		<div class="add-table">
			<a class="btn ink-reaction btn-primary" href="{{route('site::getaddarticle')}}"><span class="fa fa-plus"></span></a>
		</div>
		<hr class="ruler-xxl"/>
