        	...
        	<div class="col-lg-12">
    			<div class="card card-underline">
    			    <div class="card-head card-head-lg">
						<header>Quản lý thông tin nhóm hội đồng	<span style="color: #311B92;font-weight: bold;">
							<select name="job" id="job" onchange="window.location.href=this.value">
	    				        <option value="{{route('council_group::')}}" @if(!$intertime_id) selected @endif >-- Tất cả --</option>
	    				        @foreach( $intership_time as $opt )
	    				        <option value="{{route('council_group::',['id'=>$opt->id])}}" @if($opt->id == $intertime_id) selected @endif >{{$opt->intertime_name}}</option>
	    				        @endforeach
	    				    </select></span>
						</header>
						<div class="tools">
    				...
    				<div class="card-body">
    					<div class="table-responsive">
    						<table class="table no-margin" id="datatable1">
    							...
    							<tbody>
    							    @foreach( $council_group as $key => $item )
    							    <tr>
    									<td>{{$key+1}}</td>
    									<td>{{$item->name}}</td>
    									<td>{{$item->intership_time->intertime_name	}}</td>
    									<td>
    									    @if( \Auth::user()->has_permission( 'council_group::view' ) )
    									    <a href="{{route('council_group::view',['id'=>$item->id])}}" class="btn ink-reaction btn-primary" data-toggle="tooltip" data-placement="top" data-original-title="Thông tin nhóm hội đồng"><span class="glyphicon glyphicon-eye-open"></span></a>
    									    @endif
    									    @if( \Auth::user()->has_permission( 'council::' ) )
    									    <a href="{{route('council::',['id'=>$item->id])}}" class="btn ink-reaction btn-primary" data-toggle="tooltip" data-placement="top" data-original-title="Danh sách hội đồng trong nhóm"><span class="md md-format-list-numbered"></span></a>
    									    @endif
    									    @if( \Auth::user()->has_permission( 'council_group::edit' ) )
    									    <a href="{{route('council_group::edit',['id'=>$item->id])}}" class="btn ink-reaction btn-primary"><span class="glyphicon glyphicon-pencil" data-toggle="tooltip" data-placement="top" data-original-title="Sửa"></span></a>
    									    @endif
    									    @if( \Auth::user()->has_permission( 'council_group::delete' ) )
    									    <a href="{{route('council_group::delete',['id'=>$item->id])}}" class="btn ink-reaction btn-primary remove-btn"><span class="glyphicon glyphicon-remove" data-toggle="tooltip" data-placement="top" data-original-title="Xóa"></span></a>
    									    @endif
    									</td>
    								</tr>
    							    @endforeach
